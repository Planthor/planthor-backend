...
public class UpdatePlanCommandHandlerTests
{
    ...
    public async Task Handle_MemberNotFound_ThrowsKeyNotFoundException()
    {
        ...
        await Assert.ThrowsAsync<KeyNotFoundException>(() => _handler.Handle(new UpdatePersonalPlanCommand("user1", Guid.NewGuid(), "km", 100, 50, new DateTimeOffset(2024,1,1,0,0,0,TimeSpan.Zero), new DateTimeOffset(2025,1,1,0,0,0,TimeSpan.Zero), "Custom"), CancellationToken.None));
    }
    ...
    public async Task Handle_PlanNotFound_ThrowsKeyNotFoundException()
    {
        ...
        await Assert.ThrowsAsync<KeyNotFoundException>(() => _handler.Handle(new UpdatePersonalPlanCommand("user1", planId, "km", 100, 50, new DateTimeOffset(2024,1,1,0,0,0,TimeSpan.Zero), new DateTimeOffset(2025,1,1,0,0,0,TimeSpan.Zero), "Custom"), CancellationToken.None));
    }
    ...
    public async Task Handle_ValidRequest_UpdatesPlan()
    {
        ...
        var result = await _handler.Handle(new UpdatePersonalPlanCommand("user1", planId, "mi", 50, 20, new DateTimeOffset(2024,2,1,0,0,0,TimeSpan.Zero), new DateTimeOffset(2025,2,1,0,0,0,TimeSpan.Zero), "Custom"), CancellationToken.None);
    ...
    public async Task Handle_ValidRequestWithZeroTarget_ReturnsZeroProgress()
    {
        ...
        var result = await _handler.Handle(new UpdatePersonalPlanCommand("user1", planId, "mi", 0, 0, new DateTimeOffset(2024,2,1,0,0,0,TimeSpan.Zero), new DateTimeOffset(2025,2,1,0,0,0,TimeSpan.Zero), "Custom"), CancellationToken.None);
